...
func (d *dummyLLM) Generate(context.Context, *model.Request) (*message.Message, error) {
	return message.NewAssistantMessage("Response from agentgrapgenerator test."), nil
}

func (d *dummyLLM) Stream(ctx context.Context, req *model.Request) *model.EventStream {
	msg := message.NewAssistantMessage("Response from agentgrapgenerator test.")
...
func newTestAgent(t *testing.T, name, description string, agentType agent.Type, subAgents []agent.Agent, tools []tool.Tool) agent.Agent {
	var a agent.Agent
	var err error
	...
	case agent.TypeCustomAgent, agent.TypeLLMAgent:
...
func (m *mockTool) Execute(tool.Context, map[string]any) (any, *tool.Control, error) { return nil, nil, nil }
...
func TestNodeName(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "agent",
			instance: newTestAgent(t, "TestAgent", "", agent.TypeCustomAgent, nil, nil),
			expected: "TestAgent",
		},
		{
			name:     "tool",
...
func TestNodeCaption(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "llm agent",
			instance: newTestAgent(t, "LLMAgent", "", agent.TypeLLMAgent, nil, nil),
			expected: "\"🤖 LLMAgent\"",
		},
		{
			name:     "sequential agent",
			instance: newTestAgent(t, "SeqAgent", "", agent.TypeSequentialAgent, nil, nil),
			expected: "\"SeqAgent (SequentialAgent)\"",
		},
		{
			name:     "loop agent",
			instance: newTestAgent(t, "LoopAgent", "", agent.TypeLoopAgent, nil, nil),
			expected: "\"LoopAgent (LoopAgent)\"",
		},
		{
			name:     "parallel agent",
			instance: newTestAgent(t, "ParAgent", "", agent.TypeParallelAgent, nil, nil),
...
func TestEdgeHighlighted(t *testing.T) {
	tests := []struct {
		...
		to               string
		highlightedPairs [][]string
		expected         edgeHighlightDirection
	}{
		{
		...
		},
		{
			...
			to:               "A",
			highlightedPairs: [][]string{{"A", "B"}},
			expected:         edgeHighlightBackward,
		},
		{
			name: "no match",
			from: "C",
...
func TestDrawNode(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			agent:            newTestAgent(t, "MyAgent", "", agent.TypeCustomAgent, nil, nil),
			highlightedPairs: [][]string{},
			expected: gographviz.Attrs{
				"color":     LightGray,
				"label":     "\"🤖 MyAgent\"",
		...
		},
		{
			...
			agent:            newTestAgent(t, "HighlightedAgent", "", agent.TypeCustomAgent, nil, nil),
...
func TestDrawClusterNode(t *testing.T) {
	...
	parentGraph := graph
	visitedNodes := make(map[string]bool)
	agent := newTestAgent(t, "MyClusterAgent", "", agent.TypeSequentialAgent, nil, nil)
...
func TestDrawCluster(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			visitedNodes := make(map[string]bool)
			subAgent1 := newTestAgent(t, "SubAgent1", "", agent.TypeLLMAgent, nil, nil)
			subAgent2 := newTestAgent(t, "SubAgent2", "", agent.TypeLLMAgent, nil, nil)
...
func TestBuildGraph(t *testing.T) {
	...
	subAgent1 := newTestAgent(t, "SubAgent1", "", agent.TypeLLMAgent, nil, []tool.Tool{tool1})
	subAgent2 := newTestAgent(t, "SubAgent2", "", agent.TypeLLMAgent, nil, nil)
	mainAgent := newTestAgent(t, "MainAgent", "", agent.TypeLLMAgent, []agent.Agent{subAgent1, subAgent2}, []tool.Tool{tool2})
